...
def blds_perpendicular(theta, sample, omega_m):
    ...
    prefactor = -(fc * c0**2 * Vts**2)/(16 * np.pi * epsilon0 * kc)
    ...
    for index, omega in enumerate(omega_m):
        ...
        omegaP = omega_c + omega
        omegaM = omega_c - omega

        DeltaK2 = 0.50 * (omega_c/omegaP) * K(2, theta, sample, omegaP, np.real) + \
                  0.50 * (omega_c/omegaM) * K(2, theta, sample, omegaM, np.real)

        result[index] = (prefactor * DeltaK2).to('Hz')
...
def blds_perpendicular_jit(theta, sample, omega_m):
    ...
    for index, omega in enumerate(omega_m.to('Hz').magnitude):
        ...
        DeltaK2 = 0.50 * (omega_c/omegaP) * K_jit(2, theta, sample, omegaP, False) + \
                  0.50 * (omega_c/omegaM) * K_jit(2, theta, sample, omegaM, False)

# dissipationtheory12e.py
# Author: John A. Marohn (jam99@cornell.edu)
# Date: 2025-09-01
# Summary: Revise the KmatrixIII_jit function in dissipation11e.py to implement the improved 
# integration algorithm developed in dissipation-theory--Study-63.ipynb. 
#
# 
# Functions:
#
...
@jit(nopython=True)
def rpI_jit(y, omega, omega0, zr, kD, hs, es, ed):
    """Fresnel coefficient for Sample I object:
    ...
    Omega = omega/omega0
    theta1 = complex(1,0) * np.sqrt(y**2 * (hs / (zr * 1.0e-9) )**2 + (hs * kD)**2 * (1/es + complex(0,1) * Omega))
        ...
        for j, sje in enumerate(sj):
            ...
            for index in np.arange(len(wb)-1):
                ...
                for m, w in enumerate(w_vector):
                    ...
                    t3 = rpI_jit(t0, omega, omega0, zjkref, kD, hs, es, ed)
...
@jit(nopython=True)
def rpII_jit(y, omega, omega0, zr, kD, hd, ed, es):
    """Fresnel coefficient for Sample II object:
    ...
    Omega = omega/omega0
    theta2 = complex(1,0) * y * hd / (zr * 1.0e-9)
    k_over_eta = y / np.sqrt(y**2 + (zr * 1e-9 * kD)**2 * (1/es + complex(0,1) * Omega))
        ...
        for j, sje in enumerate(sj):
            ...
            for index in np.arange(len(wb)-1):
                ...
                for m, w in enumerate(w_vector):
                    ...
                    t1 = np.exp(w - t0)
                    t2 = scipy.special.j0(t0 * x)
                    t3 = rpII_jit(t0, omega, omega0, zjkref, kD, hd, ed, es)
